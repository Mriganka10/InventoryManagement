...
def verify_email(token: str = ""):
    ...
    return RedirectResponse("/login?verified=1", 303)
...
<script>const r=document.querySelector('#request-form'),o=document.querySelector('#otp-form'),s=document.querySelector('#status');if(new URLSearchParams(location.search).get('verified'))s.textContent='Email verified. You can now request your OTP.';r.onsubmit=async e=>{e.preventDefault();s.textContent='Sending…';const x=await fetch('/api/auth/request-otp',{method:'POST',body:new FormData(r)}),p=await x.json();if(!x.ok){o.classList.add('hidden');s.textContent=p.detail||'Unable to send OTP';return}document.querySelector('#otp-email').value=p.email;o.classList.remove('hidden');s.textContent=p.dev_otp?'Local test OTP: '+p.dev_otp:'OTP accepted for delivery to '+p.email+'. Check Inbox and Spam.';};</script></body></html>"""
